...
export class PlanBuilderCalendarComponent implements OnInit {
  ...
  getMinMaxWorkouts() {
    if (this.dataService.calendar_list == undefined || this.dataService.calendar_list == null) return;

    this.min_workouts = 7;
    this.max_workouts = 0;
    this.dataService.calendar_list.map(week => {
      let workouts = 0;
      week.map(plan => {
        if (plan == undefined || plan == null || plan == 'null' || plan == '') return;
        workouts++;
      })

      this.min_workouts = Math.min(this.min_workouts, workouts);
      this.max_workouts = Math.max(this.max_workouts, workouts);
    })
  }
    ...
    onSaveProgram(program) {
      ...
      this.http.post('program/restapi?info=save-program', {calendar : this.encodeCalendarData(this.calendar_list), program : program, program_id: this.dataService.cur_program_id}, {
          ...
          if (resp != null && resp['success'] != null) {
            ...
            this.program['id'] = resp['program_id'];
          }
        });
    }
}
