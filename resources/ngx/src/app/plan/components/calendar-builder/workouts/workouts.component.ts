...
export class PlanBuilderWorkoutsComponent implements OnInit {
  ...
  constructor(private spinner: NgxSpinnerService, private http: HttpClient, private messageService: MessageService, private readonly applicationService: ApplicationService) {
  ...
  onSaveWorkout() {
    ...
    this.spinner.show();
    this.http.post('add/workout', {data: this.exercise_list, planId: this.curWorkout}, {
      headers: new HttpHeaders({
        'Content-Type': 'application/json',
      })
    }).subscribe(resp => {
