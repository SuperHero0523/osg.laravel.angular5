<div class="program_designer home">
<section id="main-call-to-action" class="per-tra-bg">
   <img src="images/program_design_banner.png" class="w-100 img-fluid mx-auto d-block" alt="">
   <div class="content-cta">
      <div class="container h-100">
         <div class="row justify-content-center align-items-center h-100">         
            <div class="col-12 col-sm-12 col-md-12 col-lg-10 per-tra-img">
            <img class="pro-des-img" src="{{ url('images/icons/program-designer.png') }}" alt="personal Trainer">
               <h2><small>I AM A</small>  PROGRAM DESIGNER</h2>
               <div class="stars">
                  <img src="images/stars.png" class="img-fluid d-block mx-auto" alt="">
                  <p>Do you want to release your potential as a personal trainer? In order to be an eminent personal trainer you need a strong theoretical background, and a great deal of experience. The latter takes many years to accomplish. What if you could use the experience from others in your own favor?</p>
               </div>
               <div class="row justify-content-center mt-4">
                  <div class="col-12 col-sm-12 col-md-10 col-lg-8">
                     <form class="get-started" method="GET" action="">
                        <div class="form-group row">
                           <div class="col-12 col-sm-6 col-lg-7"><input type="email" id="e-mail" name="email" placeholder="E-mail address" class="form-control py-3"></div>
                           <div class="col-12 col-sm-6 col-lg-5"><button type="submit" class="btn btn-green-gradient btn-block px-5 py-3 mt-3 mt-sm-0">Get started</button></div>
                        </div>
                     </form>
                  </div>
               </div>
            </div>
         </div>
      </div>      
   </div>
</section>
<!-- benifit -->
<div class="benefit_bg">
<div class="benefit">
    <h4>BENEFITS</h4>

    <div class="row">
        <div class="benefit_build">
            <img src="images/benefit_build_img.png" alt="images">
            <p>
                Register your clients as strength athletes, and follow them up closely. Let Optimal Strength Gains find the best suited strength
                training programs for your clients, based on accumulated and objective data collected through our Experience banks.
                This will free up your time, and let you focus more on other important factors like technique, diet, motivation and
                more.
            </p>
        </div>
        <div class="benefit_fdbck">
            <img src="images/benefit_fdbck_img.png" alt="images">
            <p>
                Follow up and interact with your clients on their strength athlete profiles. The Personal trainer profile is structured so
                that your clients can take a more active approach, reducing your own time in regards to both administrative work and
                marketing.
            </p>
        </div>
        <div class="benefit_strength">
            <img src="images/benefit_strength_img.png" alt="images">
            <p>
                Follow up and interact with your clients on their strength athlete profiles. The Personal trainer profile is structured so
                that your clients can take a more active approach, reducing your own time in regards to both administrative work and
                marketing.
            </p>
        </div>
    </div>
</div>
</div>
<!-- benifit -->

<!-- calander -->
<div class="calendar_build">
			<div class="container">
				<div class="build_content">
					<h3>THE CALENDAR BUILDER</h3>
					<p>Build strength training programs in the calendar builder, by drag and drop exercises and creating different sessions,
						and then drag and drop sessions into the calendar. In the calendar builder you will create static programs, which means
						that for example Session 1 has the same variables in every week throughout the entire program.</p>
				</div>
				<img src="images/calendar_build_img.png" alt="">

			</div>
		</div>
<!-- calander -->

<!-- progress build -->
<div class="progress_build">
			<div class="container">
				<div class="row">
					<div class="progress_build_lft">
						<h3>THE PROGRESSION BUILDER</h3>
						<p>strategy in the progression builder. This can be done by gradually increasing the intensity, volume or frequency. Insert
							some light sessions or deload weeks every now and then to ensure proper fatigue management, recovery and adoption,
							which is especially important before the final one repetition maximum (1 RM) test in the final week. The latter means
							that you need to choose test exercise(s) both in the first and last week of the program, to calculate the strength
							athletes progression or results (percentage increase in 1 RM). Publish the program and make it available for the strength
							athletes.
						</p>
					</div>
					<div class="progress_build_img">
						<img src="images/progress_build_img.png" alt="">
					</div>
				</div>
			</div>
		</div>
<!-- progress build -->

<!-- publish program -->
<div class="publish_program">

			<div class="container">
				<div class="publish_content">
					<h3>PUBLISHED PROGRAMS</h3>
					<p>The published programs table shows an overview of all of your published programs, and variables like program name, published
						date, number of completions, category and training split.</p>
				</div>
				<div class="publish_table">
					<table width="100%" border="1" cellspacing="0" cellpadding="0">
						<thead>
							<tr>
								<td>PROGRAM:</td>
								<td>GVT</td>
								<td>5x5</td>
								<td>STARTING STRENGTH</td>
								<td>X-SHAPE</td>
								<td>ATHLETIC</td>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>PUBLISHED DATE:</td>
								<td>01.07.18</td>
								<td>02.05.17</td>
								<td>15.02.18</td>
								<td>12.12.17</td>
								<td>10.10.17</td>
							</tr>
							<tr>
								<td>NO. OF COMPLETIONS:</td>
								<td>352</td>
								<td>386</td>
								<td>486</td>
								<td>689</td>
								<td>796</td>
							</tr>
							<tr>
								<td>LENGTH WEEKS:</td>
								<td>12</td>
								<td>9</td>
								<td>16</td>
								<td>22</td>
								<td>12</td>
							</tr>
							<tr>
								<td>CATEGORY:</td>
								<td>Hypertrophy</td>
								<td>Strength</td>
								<td>Strength</td>
								<td>Hypertrophy</td>
								<td>Hypertrophy</td>
							</tr>
							<tr>
								<td>TRAINING SPLIT:</td>
								<td>3</td>
								<td>2</td>
								<td>4</td>
								<td>5</td>
								<td>3</td>
							</tr>
						</tbody>
					</table>
				</div>
			</div>
		</div>
<!-- publish program -->

<!-- bank experience -->
<div class="bank_experience">
			<div class="container">
			<div class="row">
				<div class="bnk_exp_content">
					<h3>EXPERIENCE BANK</h3>
					<p>The Experience bank will continuously gather information from all the strength athletes that have completed your program. 
						</p>
					<p>Each program has one Experience bank for each Test exercise, which in turn are divided into one table for men and one table for female.</p>
					<p>Every program has one or more test exercises, which are conducted in the programs first and last training week. By comparing the 1 RM pre- and posttest results for the test exercise (ex: Bench press), we can calculate percentage increase for an individual. For example if a strength athlete’s pretest was 100 kg and posttest was 105 kg, the lifters percentage increase in bench press was 5 percent. </p>
					<p>All the individual percentage increase results are collected in a table that consists of levels from untrained to elite, and different weight classes. The number in a cell belongs to a specific weight class 
							interval combined with a level (for example 90-99 kg and level: 
							intermediate), and shows the median percentage increase result. The median (average) is the middle value when all the strength athlete’s percentage increase values are listed in numerical order from the lowest to the highest. The median also indicates the results a strength athlete with his or her body weight and level, can expect to achieve in the test exercise, if choosing to follow and complete your program.
							</p>
					<p>By comparing the median for a specific level and body weight i
							nterval for different programs made by different Program designers, strength athletes can find the most optimal strength training 
							programs. This is done in the Experience Bank search section.</p>
				</div>
				<div class="bnk_exp_table">
						<div class="row">
								<div class="exp_table_parent">
								   <div class="table-responsive">
									  <h4>TEST EXERCISE: BENCH PRESS (MEN) <span>- PERCENTAGE INCREASE</span></h4>
									  <table class="table">
										 <thead>
											<tr>
											   <th>BODY WEIGHT</th>
											   <th>UNTRAINED</th>
											   <th>NOVICE</th>
											   <th>INTERMEDIATE</th>
											   <th>ADVANCED</th>
											   <th>ELITE</th>
											</tr>
										 </thead>
										 <tbody>
											<tr>
											   <td>50-59</td>
											   <td>8,21</td>
											   <td>8,01</td>
											   <td>7,98</td>
											   <td>7,10</td>
											   <td>6,21</td>
											</tr>
											<tr>
											   <td>60-69</td>
											   <td>9,11</td>
											   <td>9,25</td>
											   <td>8,12</td>
											   <td>6,89</td>
											   <td>5,43</td>
											</tr>
											<tr>
											   <td>70-79</td>
											   <td>7,65</td>
											   <td>7,66</td>
											   <td>6,48</td>
											   <td>5,21</td>
											   <td>3,21</td>
											</tr>
											<tr>
											   <td>80-89</td>
											   <td>6,54</td>
											   <td>6,68</td>
											   <td>7,21</td>
											   <td>5,12</td>
											   <td>4,47</td>
											</tr>
											<tr>
											   <td>90-99</td>
											   <td>4,41</td>
											   <td>5,54</td>
											   <td>6,81</td>
											   <td>3,41</td>
											   <td>2,21</td>
											</tr>
											<tr>
											   <td>100-109</td>
											   <td>6,87</td>
											   <td>5,54</td>
											   <td>4,83</td>
											   <td>2,19</td>
											   <td>1,12</td>
											</tr>
											<tr>
											   <td>110-119</td>
											   <td>8,57</td>
											   <td>7,23</td>
											   <td>6,57</td>
											   <td>4,12</td>
											   <td>3,49</td>
											</tr>
											<tr>
											   <td>120+</td>
											   <td>6,54</td>
											   <td>0,99</td>
											   <td>2,19</td>
											   <td>2,48</td>
											   <td>3,59</td>
											</tr>
										 </tbody>
									  </table>
								   </div>
								</div>
								<div class="exp_table_parent">
								   <div class="table-responsive">
									  <h4>TEST EXERCISE: BENCH PRESS (WOMEN) <span>- PERCENTAGE INCREASE</span></h4>
									  <table class="table">
										 <thead>
											<tr>
											   <th>BODY WEIGHT</th>
											   <th>UNTRAINED</th>
											   <th>NOVICE</th>
											   <th>INTERMEDIATE</th>
											   <th>ADVANCED</th>
											   <th>ELITE</th>
											</tr>
										 </thead>
										 <tbody>
											<tr>
											   <td>50-59</td>
											   <td>8,21</td>
											   <td>8,01</td>
											   <td>7,98</td>
											   <td>7,10</td>
											   <td>6,21</td>
											</tr>
											<tr>
											   <td>60-69</td>
											   <td>9,11</td>
											   <td>9,25</td>
											   <td>8,12</td>
											   <td>6,89</td>
											   <td>5,43</td>
											</tr>
											<tr>
											   <td>70-79</td>
											   <td>7,65</td>
											   <td>7,66</td>
											   <td>6,48</td>
											   <td>5,21</td>
											   <td>3,21</td>
											</tr>
											<tr>
											   <td>80-89</td>
											   <td>6,54</td>
											   <td>6,68</td>
											   <td>7,21</td>
											   <td>5,12</td>
											   <td>4,47</td>
											</tr>
											<tr>
											   <td>90-99</td>
											   <td>4,41</td>
											   <td>5,54</td>
											   <td>6,81</td>
											   <td>3,41</td>
											   <td>2,21</td>
											</tr>
											<tr>
											   <td>100-109</td>
											   <td>6,87</td>
											   <td>5,54</td>
											   <td>4,83</td>
											   <td>2,19</td>
											   <td>1,12</td>
											</tr>
											<tr>
											   <td>110-119</td>
											   <td>8,57</td>
											   <td>7,23</td>
											   <td>6,57</td>
											   <td>4,12</td>
											   <td>3,49</td>
											</tr>
											<tr>
											   <td>120+</td>
											   <td>6,54</td>
											   <td>0,99</td>
											   <td>2,19</td>
											   <td>2,48</td>
											   <td>3,59</td>
											</tr>
										 </tbody>
									  </table>
								   </div>
								</div>
								<div class="exp_table_parent">
								   <div class="table-responsive">
									  <h4>TEST EXERCISE: SQUAT (MEN) <span>- PERCENTAGE INCREASE</span></h4>
									  <table class="table">
										 <thead>
											<tr>
											   <th>BODY WEIGHT</th>
											   <th>UNTRAINED</th>
											   <th>NOVICE</th>
											   <th>INTERMEDIATE</th>
											   <th>ADVANCED</th>
											   <th>ELITE</th>
											</tr>
										 </thead>
										 <tbody>
											<tr>
											   <td>50-59</td>
											   <td>8,21</td>
											   <td>8,01</td>
											   <td>7,98</td>
											   <td>7,10</td>
											   <td>6,21</td>
											</tr>
											<tr>
											   <td>60-69</td>
											   <td>9,11</td>
											   <td>9,25</td>
											   <td>8,12</td>
											   <td>6,89</td>
											   <td>5,43</td>
											</tr>
											<tr>
											   <td>70-79</td>
											   <td>7,65</td>
											   <td>7,66</td>
											   <td>6,48</td>
											   <td>5,21</td>
											   <td>3,21</td>
											</tr>
											<tr>
											   <td>80-89</td>
											   <td>6,54</td>
											   <td>6,68</td>
											   <td>7,21</td>
											   <td>5,12</td>
											   <td>4,47</td>
											</tr>
											<tr>
											   <td>90-99</td>
											   <td>4,41</td>
											   <td>5,54</td>
											   <td>6,81</td>
											   <td>3,41</td>
											   <td>2,21</td>
											</tr>
											<tr>
											   <td>100-109</td>
											   <td>6,87</td>
											   <td>5,54</td>
											   <td>4,83</td>
											   <td>2,19</td>
											   <td>1,12</td>
											</tr>
											<tr>
											   <td>110-119</td>
											   <td>8,57</td>
											   <td>7,23</td>
											   <td>6,57</td>
											   <td>4,12</td>
											   <td>3,49</td>
											</tr>
											<tr>
											   <td>120+</td>
											   <td>6,54</td>
											   <td>0,99</td>
											   <td>2,19</td>
											   <td>2,48</td>
											   <td>3,59</td>
											</tr>
										 </tbody>
									  </table>
								   </div>
								</div>
								<div class="exp_table_parent">
								   <div class="table-responsive">
									  <h4>TEST EXERCISE: SQUAT (WOMEN) <span>- PERCENTAGE INCREASE</span></h4>
									  <table class="table">
										 <thead>
											<tr>
											   <th>BODY WEIGHT</th>
											   <th>UNTRAINED</th>
											   <th>NOVICE</th>
											   <th>INTERMEDIATE</th>
											   <th>ADVANCED</th>
											   <th>ELITE</th>
											</tr>
										 </thead>
										 <tbody>
											<tr>
											   <td>50-59</td>
											   <td>8,21</td>
											   <td>8,01</td>
											   <td>7,98</td>
											   <td>7,10</td>
											   <td>6,21</td>
											</tr>
											<tr>
											   <td>60-69</td>
											   <td>9,11</td>
											   <td>9,25</td>
											   <td>8,12</td>
											   <td>6,89</td>
											   <td>5,43</td>
											</tr>
											<tr>
											   <td>70-79</td>
											   <td>7,65</td>
											   <td>7,66</td>
											   <td>6,48</td>
											   <td>5,21</td>
											   <td>3,21</td>
											</tr>
											<tr>
											   <td>80-89</td>
											   <td>6,54</td>
											   <td>6,68</td>
											   <td>7,21</td>
											   <td>5,12</td>
											   <td>4,47</td>
											</tr>
											<tr>
											   <td>90-99</td>
											   <td>4,41</td>
											   <td>5,54</td>
											   <td>6,81</td>
											   <td>3,41</td>
											   <td>2,21</td>
											</tr>
											<tr>
											   <td>100-109</td>
											   <td>6,87</td>
											   <td>5,54</td>
											   <td>4,83</td>
											   <td>2,19</td>
											   <td>1,12</td>
											</tr>
											<tr>
											   <td>110-119</td>
											   <td>8,57</td>
											   <td>7,23</td>
											   <td>6,57</td>
											   <td>4,12</td>
											   <td>3,49</td>
											</tr>
											<tr>
											   <td>120+</td>
											   <td>6,54</td>
											   <td>0,99</td>
											   <td>2,19</td>
											   <td>2,48</td>
											   <td>3,59</td>
											</tr>
										 </tbody>
									  </table>
								   </div>
								</div>
								<div class="exp_table_parent">
								   <div class="table-responsive">
									  <h4>TEST EXERCISE: DEADLIFT (MEN) <span>- PERCENTAGE INCREASE</span></h4>
									  <table class="table">
										 <thead>
											<tr>
											   <th>BODY WEIGHT</th>
											   <th>UNTRAINED</th>
											   <th>NOVICE</th>
											   <th>INTERMEDIATE</th>
											   <th>ADVANCED</th>
											   <th>ELITE</th>
											</tr>
										 </thead>
										 <tbody>
											<tr>
											   <td>50-59</td>
											   <td>8,21</td>
											   <td>8,01</td>
											   <td>7,98</td>
											   <td>7,10</td>
											   <td>6,21</td>
											</tr>
											<tr>
											   <td>60-69</td>
											   <td>9,11</td>
											   <td>9,25</td>
											   <td>8,12</td>
											   <td>6,89</td>
											   <td>5,43</td>
											</tr>
											<tr>
											   <td>70-79</td>
											   <td>7,65</td>
											   <td>7,66</td>
											   <td>6,48</td>
											   <td>5,21</td>
											   <td>3,21</td>
											</tr>
											<tr>
											   <td>80-89</td>
											   <td>6,54</td>
											   <td>6,68</td>
											   <td>7,21</td>
											   <td>5,12</td>
											   <td>4,47</td>
											</tr>
											<tr>
											   <td>90-99</td>
											   <td>4,41</td>
											   <td>5,54</td>
											   <td>6,81</td>
											   <td>3,41</td>
											   <td>2,21</td>
											</tr>
											<tr>
											   <td>100-109</td>
											   <td>6,87</td>
											   <td>5,54</td>
											   <td>4,83</td>
											   <td>2,19</td>
											   <td>1,12</td>
											</tr>
											<tr>
											   <td>110-119</td>
											   <td>8,57</td>
											   <td>7,23</td>
											   <td>6,57</td>
											   <td>4,12</td>
											   <td>3,49</td>
											</tr>
											<tr>
											   <td>120+</td>
											   <td>6,54</td>
											   <td>0,99</td>
											   <td>2,19</td>
											   <td>2,48</td>
											   <td>3,59</td>
											</tr>
										 </tbody>
									  </table>
								   </div>
								</div>
								<div class="exp_table_parent">
								   <div class="table-responsive">
									  <h4>TEST EXERCISE: DEADLIFT (WOMEN) <span>- PERCENTAGE INCREASE</span></h4>
									  <table class="table">
										 <thead>
											<tr>
											   <th>BODY WEIGHT</th>
											   <th>UNTRAINED</th>
											   <th>NOVICE</th>
											   <th>INTERMEDIATE</th>
											   <th>ADVANCED</th>
											   <th>ELITE</th>
											</tr>
										 </thead>
										 <tbody>
											<tr>
											   <td>50-59</td>
											   <td>8,21</td>
											   <td>8,01</td>
											   <td>7,98</td>
											   <td>7,10</td>
											   <td>6,21</td>
											</tr>
											<tr>
											   <td>60-69</td>
											   <td>9,11</td>
											   <td>9,25</td>
											   <td>8,12</td>
											   <td>6,89</td>
											   <td>5,43</td>
											</tr>
											<tr>
											   <td>70-79</td>
											   <td>7,65</td>
											   <td>7,66</td>
											   <td>6,48</td>
											   <td>5,21</td>
											   <td>3,21</td>
											</tr>
											<tr>
											   <td>80-89</td>
											   <td>6,54</td>
											   <td>6,68</td>
											   <td>7,21</td>
											   <td>5,12</td>
											   <td>4,47</td>
											</tr>
											<tr>
											   <td>90-99</td>
											   <td>4,41</td>
											   <td>5,54</td>
											   <td>6,81</td>
											   <td>3,41</td>
											   <td>2,21</td>
											</tr>
											<tr>
											   <td>100-109</td>
											   <td>6,87</td>
											   <td>5,54</td>
											   <td>4,83</td>
											   <td>2,19</td>
											   <td>1,12</td>
											</tr>
											<tr>
											   <td>110-119</td>
											   <td>8,57</td>
											   <td>7,23</td>
											   <td>6,57</td>
											   <td>4,12</td>
											   <td>3,49</td>
											</tr>
											<tr>
											   <td>120+</td>
											   <td>6,54</td>
											   <td>0,99</td>
											   <td>2,19</td>
											   <td>2,48</td>
											   <td>3,59</td>
											</tr>
										 </tbody>
									  </table>
								   </div>
								</div>
							 </div>
				</div>
			</div>
		</div>
		</div>
<!-- bank experience -->

<!-- reviews -->
<div class="reviews">
			<div class="review_content">
				<h3>REVIEWS</h3>
				<p>In the review section the strength athlete’s and personal trainer can give feedback on their experience with your program.
					This enables you to learn and improve your work. </p>
			</div>
			<div class="review_chart">
				<div class="user_review">
					<img src="images/avatar_img.png" alt="">
					<h5>Alex James</h5>
					<p>Best coach I have worked with! Talented and genuin</p>
					 <div class="rating">
						<span>5.0</span>
						<img src="images/green_star_img.png" alt="">
						<img src="images/green_star_img.png" alt="">
						<img src="images/green_star_img.png" alt="">
						<img src="images/green_star_img.png" alt="">
						<img src="images/green_star_img.png" alt="">
					</div>
				</div>
				<div class="user_review">
                    <img src="images/avatar_img.png" alt="">
                    <h5>Sara Nicole</h5>
                    <p>Best coach I have worked with! Talented and genuin</p>
                        <div class="rating">
                        <span>5.0</span>
                    <img src="images/green_star_img.png" alt="">
                    <img src="images/green_star_img.png" alt="">
                    <img src="images/green_star_img.png" alt="">
                    <img src="images/green_star_img.png" alt="">
                    <img src="images/green_star_img.png" alt="">
                    </div>
				</div>
				<div class="user_review">
                    <img src="images/avatar_img.png" alt="">
                    <h5>Alexsay Ivanov</h5>
                    <p>Best coach I have worked with! Talented and genuin</p>
                        <div class="rating">
                        <span>3.0</span>
                        <img src="images/yellow_star_img.png" alt="">
                        <img src="images/yellow_star_img.png" alt="">
                        <img src="images/yellow_star_img.png" alt="">
                        <img src="images/gray_star_img.png" alt="">
                        <img src="images/gray_star_img.png" alt="">
                    </div>
				</div>
				<div class="user_review">
                    <img src="images/avatar_img.png" alt="">
                    <h5>James Smith</h5>
                    <p>Best coach I have worked with! Talented and genuin</p>
                        <div class="rating">
                        <span>3.0</span>
                        <img src="images/yellow_star_img.png" alt="">
                        <img src="images/yellow_star_img.png" alt="">
                        <img src="images/yellow_star_img.png" alt="">
                        <img src="images/gray_star_img.png" alt="">
                        <img src="images/gray_star_img.png" alt="">
                    </div>
				</div>
				<div class="user_review">
                    <img src="images/avatar_img.png" alt="">
                    <h5>Louis Hamstrings</h5>
                    <p>Best coach I have worked with! Talented and genuin</p>
                    <div class="rating">
                        <span>5.0</span>
                    <img src="images/green_star_img.png" alt="">
                    <img src="images/green_star_img.png" alt="">
                    <img src="images/green_star_img.png" alt="">
                    <img src="images/green_star_img.png" alt="">
                    <img src="images/green_star_img.png" alt="">
                    </div>
				</div>	
			</div>
		</div>
<!-- reviews -->

<!-- program design -->
<div class="program_design">
			<div class="container">
				<div class="program_content">
					<h3>PROGRAM DESIGN</h3>
					<p>The program design summarizes how the strength training program is structured, concerning the most important variables,
						like training volume, intensity, frequency and more. For the strength athlete the program design reveals why he or
						she responds to it, for example exceptionally good like a high responder, or exceptionally bad like a low responder.
						Exercise selection isn’t necessarily the most important variable in a training design. </p>
				</div>
				<div class="table-top-program">
					<div class="table-responsive">
						<table class="table">
							<tbody>
								<tr>
									<td rowspan="2">Muscle Group</td>
									<td rowspan="2">Test Exercise</td>
									<td rowspan="2">Type of exercise</td>
									<td rowspan="2">Exercises</td>
									<td class="blue" colspan="2">Volume</td>
									<td class="orange" colspan="2">Intensity</td>
									<td class="green" rowspan="2">Frequency (muscle group per week)</td>
									<td rowspan="2">Progression model</td>
									<td rowspan="2">Rest periods (min-max)</td>
									<td rowspan="2">Tempo (all)</td>
								</tr>
								<tr>
									<td class="blue">Reps (min-max)</td>
									<td class="blue">Volume pr week(setxreps)</td>
									<td class="orange">% of 1RM (min-max)</td>
									<td class="orange">Reps to failure (min-max)</td>
								</tr>
								<tr>
									<td rowspan="2">Chest</td>
									<td rowspan="2">Bench Press</td>
									<td>Multi joint exercises</td>
									<td></td>
									<td class="blue"></td>
									<td class="blue"></td>
									<td class="orange"></td>
									<td class="orange"></td>
									<td class="green"></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
								<tr>
									<td>Single joint exercises</td>
									<td></td>
									<td class="blue"></td>
									<td class="blue"></td>
									<td class="orange"></td>
									<td class="orange"></td>
									<td class="green"></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
								<tr>
									<td rowspan="2">Back</td>
									<td rowspan="2">Deadlift</td>
									<td>Multi joint exercises</td>
									<td></td>
									<td class="blue"></td>
									<td class="blue"></td>
									<td class="orange"></td>
									<td class="orange"></td>
									<td class="green"></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
								<tr>
									<td>Single joint exercises</td>
									<td></td>
									<td class="blue"></td>
									<td class="blue"></td>
									<td class="orange"></td>
									<td class="orange"></td>
									<td class="green"></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
								<tr>
									<td rowspan="2">Quads</td>
									<td rowspan="2">Squat</td>
									<td>Multi joint exercises</td>
									<td></td>
									<td class="blue"></td>
									<td class="blue"></td>
									<td class="orange"></td>
									<td class="orange"></td>
									<td class="green"></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
								<tr>
									<td>Single joint exercises</td>
									<td></td>
									<td class="blue"></td>
									<td class="blue"></td>
									<td class="orange"></td>
									<td class="orange"></td>
									<td class="green"></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
								<tr>
									<td rowspan="2">Shoulders</td>
									<td rowspan="2"></td>
									<td>Multi joint exercises</td>
									<td></td>
									<td class="blue"></td>
									<td class="blue"></td>
									<td class="orange"></td>
									<td class="orange"></td>
									<td class="green"></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
								<tr>
									<td>Single joint exercises</td>
									<td></td>
									<td class="blue"></td>
									<td class="blue"></td>
									<td class="orange"></td>
									<td class="orange"></td>
									<td class="green"></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
								<tr>
									<td rowspan="2">Hamstrings</td>
									<td rowspan="2"></td>
									<td>Multi joint exercises</td>
									<td></td>
									<td class="blue"></td>
									<td class="blue"></td>
									<td class="orange"></td>
									<td class="orange"></td>
									<td class="green"></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
								<tr>
									<td>Single joint exercises</td>
									<td></td>
									<td class="blue"></td>
									<td class="blue"></td>
									<td class="orange"></td>
									<td class="orange"></td>
									<td class="green"></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
								<tr>
									<td rowspan="2">Calves</td>
									<td rowspan="2"></td>
									<td>Multi joint exercises</td>
									<td></td>
									<td class="blue"></td>
									<td class="blue"></td>
									<td class="orange"></td>
									<td class="orange"></td>
									<td class="green"></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
								<tr>
									<td>Single joint exercises</td>
									<td></td>
									<td class="blue"></td>
									<td class="blue"></td>
									<td class="orange"></td>
									<td class="orange"></td>
									<td class="green"></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
								<tr>
									<td rowspan="2">Triceps</td>
									<td rowspan="2"></td>
									<td>Multi joint exercises</td>
									<td></td>
									<td class="blue"></td>
									<td class="blue"></td>
									<td class="orange"></td>
									<td class="orange"></td>
									<td class="green"></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
								<tr>
									<td>Single joint exercises</td>
									<td></td>
									<td class="blue"></td>
									<td class="blue"></td>
									<td class="orange"></td>
									<td class="orange"></td>
									<td class="green"></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
								<tr>
									<td rowspan="2">Biceps</td>
									<td rowspan="2"></td>
									<td>Multi joint exercises</td>
									<td></td>
									<td class="blue"></td>
									<td class="blue"></td>
									<td class="orange"></td>
									<td class="orange"></td>
									<td class="green"></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
								<tr>
									<td>Single joint exercises</td>
									<td></td>
									<td class="blue"></td>
									<td class="blue"></td>
									<td class="orange"></td>
									<td class="orange"></td>
									<td class="green"></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
							</tbody>
						</table>
					</div>
				</div>

			</div>
		</div>
<!-- program design -->


<!-- training program -->
<div class="training_program">
			<div class="container">
				<div class="training_program_content">

					<h3>THE TRAINING PROGRAM</h3>
					<p>The calendar function in the Program designer profile is simply called the Training program. Here the strength athlete
						can see all the details of the training program, like training days, exercises, sets, reps, method and more. The strength
						athlete can then download the desired program to his or hers Calendar &amp; training log, ready to start the program where
						the sessions are logged on the strength athletes cellphones. </p>
				</div>

				<div class="osg-personal-trainer-booking-calendar">
					<img src="images/program_design_calendar.png" alt="calendar" class="calender_img">
					<img src="images/program_design_calendar_popup.png" alt="calendar_popup_img" class="calendar_popup_img">
				</div>
			</div>

		</div>
<!-- training program -->

<!-- news feed -->
<div class="container">
			<div class="news_feed">

				<div class="row">
					<div class="new_feed_lftcol">
						<div class="news_content">
							<h3>NEWSFEED</h3>
							<p>Communicate with your clients, give them guidance and answer questions. The Newsfeed gives you also the opportunity
								to attract new clients by marketing, blogging and by taking an active approach. </p>

						</div>


					</div>
					<div class="new_feed_rgtcol">
						<div class="post-creator">
							<div class="comment">
								<div class="row no-gutters align-items-center">
									<div class="col-auto">
										<figure>
											<img class="img-fluid d-block mx-auto" src="images/news_profile_img.png">
										</figure>
									</div>
									<div class="col">
										<textarea class="message" cols="30" id="" name="" placeholder="What’s on your mind?" rows="2"></textarea>
									</div>
								</div>
							</div>
							<div class="actions">
								<div class="row align-items-center">
									<div class="col-12 col-sm-12 col-lg-7">
										<a class="add-media" href="">
											<img alt="" class="img-fluid mr-1" src="images/camera_icon.png">Photo/Video</a>
									</div>
									<div class="col-12 col-sm-12 col-lg-5 mt-3 mt-lg-0">
										<a class="btn btn-block btn-black" href="">
											<img alt="Post" class="d-inline-block img-fluid" src="images/pencil_img.png"> POST</a>
									</div>
								</div>
							</div>
						</div>
						<div class="post">
							<div class="content">
								<div class="author">
									<div class="row no-gutters align-items-center">
										<div class="col-auto mr-2">
											<figure>
												<img class="img-fluid d-block mx-auto" src="images/avatar_img.png">
											</figure>
										</div>
										<div class="col-8 col-sm-8 col-md-6 col-lg-8">
											<div class="summary">
												<a href="#">Jeanette Wilson</a>
												<span>July 20, 2018</span>
											</div>
										</div>
										<div class="col text-right">
											<a aria-controls="popover" aria-expanded="false" class="toggle-popover" data-toggle="collapse" href="#popover-id-1" role="button">
												<i aria-hidden="true" class="fa fa-ellipsis-h"></i>
											</a>
										</div>
										<div class="collapse popover-menu" id="popover-id-1">
											<ul>
												<li>
													<a href="">Turn off notifications</a>
												</li>
												<li>
													<a href="">Edit post</a>
												</li>
												<li>
													<a href="">Delete</a>
												</li>
											</ul>
										</div>
									</div>
								</div>
								<div class="post-content">
									<p>Hi! What is the idea behind the 10 reps x 10 sets in the GVT program?</p>

									<div class="video">
										<img class="img-fluid d-block mx-auto" src="">
									</div>
								</div>
							</div>
							<div class="comments-box">
								<!---->
								<div class="comment">
									<div class="row no-gutters">
										<div class="col-auto mr-2">
											<figure>
												<img class="img-fluid d-block mx-auto" src="images/avatar_img.png">
											</figure>
										</div>
										<div class="col">
											<div class="speech-bubble">
												<div class="comment-container">
													<a href="#">Andreas Stensrud</a>
													<p>The idea is to accummulate a great deal of volume, because volume is perhaps the most important factor when
														it comes to increase muscle mass
													</p>
												</div>
												<span>30 minutes ago</span>
											</div>
										</div>
										<div class="col-auto align-self-center">
											<a aria-controls="popover" aria-expanded="false" class="toggle-popover" data-toggle="collapse" href="#popover-id-comment-1" role="button">
												<i aria-hidden="true" class="fa fa-ellipsis-h"></i>
											</a>
										</div>
										<div class="collapse popover-menu" id="popover-id-comment-1">
											<ul>
												<li>
													<a href="">Turn off notifications</a>
												</li>
												<li>
													<a href="">Edit post</a>
												</li>
												<li>
													<a href="">Delete</a>
												</li>
											</ul>
										</div>
									</div>
								</div>
								<div class="comment-creator">
									<div class="row no-gutters align-items-center">
										<div class="col-auto mr-3">
											<figure>
												<img class="img-fluid d-block mx-auto" src="images/avatar_img.png">
											</figure>
										</div>
										<div class="col">
											<div class="input-container">
												<input placeholder="Write a comment" type="text">
												<a class="add-media" href="">
													<img alt="" class="img-fluid" src="images/camera_icon2.png">
												</a>
											</div>
										</div>
									</div>
								</div>
							</div>
						</div>
					</div>
				</div>



			</div>
		</div>
<!-- news feed -->


<!-- footer form -->
<div class="container-fluid starter_log">
			<div class="row justify-content-center">
				<div class="col-lg-4 col-12 col-sm-10 col-md-8">
					<div class="text-center mb-4">
						<h2>
							<small>REACH YOUR FITNESS GOALS</small>
							<br>LET’S GET STARTED
						</h2>
					</div>
					<form method="POST" action="http://osg.fantasylab.io/register" novalidate="novalidate">
						<input type="hidden" name="_token" value="9lvD49qRK5vM8s3KjDvpwBGOVN2nj9cLsFq4IPWB">
						<div class="form-group row">
							<div class="col-12">
								<input type="text" name="name" id="name" class="form-control " value="" required="" data-msg-required="Please enter your full name">
								<label for="name">Full name *</label>
							</div>
						</div>
						<div class="form-group row">
							<div class="col-12 col-sm-12 col-md-12 col-lg-6">
								<input type="text" name="phone" id="phone" class="form-control" required="" data-msg-required="Please enter your phone">
								<label for="phone">Phone number *</label>
							</div>
							<div class="col-12 col-sm-12 col-md-12 col-lg-6 mt-3 mt-lg-0">
								<input type="email" name="email" id="email" class="form-control " value="" required="" data-msg-required="Please enter your email address">
								<label for="email">E-mail *</label>
							</div>
						</div>
						<div class="form-group row">
							<div class="col-12 col-sm-12 col-md-12 col-lg-6">
								<input type="password" class="form-control " name="password" id="password" required="" data-msg-required="Please enter a password">
								<label for="password">Password *</label>
							</div>
							<div class="col-12 col-sm-12 col-md-12 col-lg-6 mt-3 mt-lg-0">
								<input type="password" class="form-control" name="password_confirmation" id="password_confirmation" equalto="#password">
								<label for="password_confirmation">Retype password *</label>
							</div>
						</div>
						<div class="form-group row">
							<div class="col-12">
								<select name="role" class="form-control" id="role" required="">
									<option value="0">Personal Trainer</option>
									<option value="1">Program Designer</option>
									<option value="2">Strength Athlete</option>
								</select>
								<label for="role" class="labelfocus">Select type of user *</label>
							</div>
						</div>
						<div class="checkbox custom-checkbox">
							<label>
								<input type="checkbox" value="" id="agreement-checkbox">By signing up you agree to the
								<a href="http://osg.fantasylab.io/tos">Terms of Service</a> &amp;
								<a href="http://osg.fantasylab.io/privacy">Privacy Policy</a>.
								<span class="checkmark"></span>
							</label>
						</div>
						<div class="wrapper-btn text-center text-md-right mt-3">
							<button id="submit-button" value="GET STARTED" class="btn btn-green-gradient btn-block px-5 py-3">GET STARTED</button>
						</div>
					</form>
				</div>
			</div>
		</div>
<!-- footer form -->

</div>
